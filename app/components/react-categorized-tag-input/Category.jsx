...
const Category = React.createClass({
    ...
    if (this.props.addNew && !fullMatch && !this.props.single) {
      return [
        this.props.items.length > 0 ?
          <span key='cat_or' className={cx('cti__category__or')}>or</span> :
          null,
        <button
          key='add_btn'
          className={cx({'cti__category__add-item': true, 'cti-selected' : selected})}
  ...
  render() {
    ...
    return (
      <div className={cx('cti__category')}>
        <h5 className={cx('cti__category__title')}>{this.props.title}</h5>
        <div className={cx('cti__category__tags')}>
          {items}
          {addBtn}
        </div>
      </div>
