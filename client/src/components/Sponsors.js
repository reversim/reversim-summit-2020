...
const Sponsor = ({ sponsor: { name = '', logo, url, description = '', featuredJobInfo, featuredJobLink, excludeWebsite}, onEdit, onDelete}) => {
  const showJob = featuredJobLink || featuredJobInfo;
  const showJobLink = featuredJobLink && !excludeWebsite;
  const descriptionWithLink = excludeWebsite ? description : `${description} [${name}'s website](${url}).`;
  const featuredJob = showJobLink ? `${featuredJobInfo} Interested? More info [here](${featuredJobLink}).` : featuredJobInfo;
  return (
    <div key={name}>
      ...
      <Row noGutters={true}>
        <Col sm={{ size: 8, offset: 2}} className="separator pb-5 mb-5">
          <h4>
            {name}
            {onEdit && <span>
              <Button size="sm" color="primary" className="ml-2" onClick={onEdit}><i className="fa fa-pencil"/></Button>
              <Button size="sm" color="danger" className="ml-2" onClick={onDelete}><i className="fa fa-trash"/></Button>
            </span>}
          </h4>
...
class SponsorWithEdit extends React.Component {
  ...
  render() {
    ...
    return isEditing ? <SponsorForm sponsor={sponsor} onSubmit={this.onSubmit} onCancel={this.onCancel} isLoading={this.state.isLoading}/> : <Sponsor onEdit={canEdit && this.onEdit} onDelete={canEdit && this.onDelete} sponsor={sponsor} />
  }
}
    ...
    return (
      <Page title="Sponsors" {...this.props}>
        ...
        <Container>
          ...
          {user && user.isReversimTeamMember && <div className="border p-3 mb-8">
            <h3>Add sponsor</h3>
            <SponsorForm onSubmit={createSponsor} />
          </div>}
          {sponsors.map(sponsor => <SponsorWithEdit key={sponsor._id} sponsor={sponsor} canEdit={user && user.isReversimTeamMember} updateSponsor={updateSponsor} deleteSponsor={deleteSponsor}/>)}
          <WantToBe/>
        </Container>
      </Page>
    )
...
class SponsorForm extends React.Component {
  ...
  getData = (event) => {
    ...
    return pick(this.state, ['name', 'logo', 'url', 'description', 'featuredJobInfo', 'featuredJobLink', 'excludeWebsite']);
  };
    ...
    return (
      ...
      <Col className="border-right">
        <form onSubmit={e => onSubmit(this.getData(e))}>
          <Input className="mb-3" size="sm" required placeholder="Name" value={this.state.name} onChange={(e) => this.setState({ name: e.target.value })} />
          <Button className="p-relative mb-3" size="sm"><input type="file" onChange={e => {
          ...
          {!onCancel && <Button className="d-block mx-auto" color="primary" style={{width:150}} disabled={isLoading}>Submit</Button>}
          {onCancel && <div className="d-flex justify-content-around">
            <Button outline color="primary" onClick={(e) => {e.preventDefault(); onCancel()}}>Cancel</Button>
