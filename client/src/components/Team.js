...
class Team extends Component {
  ...
  teamMember = ({_id, name, oneLiner, picture, bio}) => {
    ...
    return (
      <Col
        ...
        lg="3"
        key={name}
        className={cn(s.teamMember, 'text-center mb-5 cursor-pointer')}
        onClick={() => this.onTeamMemberClick(_id)}>
        <div
          className={cn(s.teamImg, 'mx-auto mb-3')}
  ...
  render() {
    ...
    return (
      <Section title="Team">
        <Row className="mt-5">{team.map(this.teamMember)}</Row>
      </Section>
    );
  }
}
