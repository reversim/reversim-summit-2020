...
const CFPTitle = () => {
  ...
  return (
    <div className="text-center border-bottom pb-5 mb-5">
      <h1 className="my-5">{REVERSIM_SUMMIT} - Submission</h1>
      <h4 className="mb-5">Read carefully before submission!</h4>
      <h4 className="text-red">Deadline: {isToday ? 'Today!' : CFP_ENDS_STR_SHORT}</h4>
      {!isToday && (
        <div className="text-gray-600">
          <span className={daysRemaining}>{remainingDays}</span> days remaining
        </div>
      )}
    </div>
  );
