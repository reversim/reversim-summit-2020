...
const SpeakerShort = ({speaker, editable, hasLink}) => {
  ...
  const nameEl = (
    <h3>
      {name}
      {editable && (
        <Link to={`/speaker/${speaker._id}/edit`}>
  ...
  return (
    <Row className={cn('align-items-center my-4', s.speakerShort)}>
      <Col md="auto">
        <div
          className={cn(s.speakerImg, 'mx-auto mx-lg-0 mb-4 mb-lg-0')}
      ...
      <Col className="text-center text-md-left">
        ...
        <SpeakerSocialLinks
          {...speaker}
          className={cn(s.socialLinks, 'justify-content-center justify-content-md-start')}
        />
      </Col>
    </Row>
  );
